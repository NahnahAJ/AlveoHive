class PropertyApprovalMailer < ApplicationMailer
  def property_approved(property)
    ...
    mail(to: property.user.email.to_s, subject: 'Property Approved on AlveoHive') do |format|
      format.text
    end
  end
  ...
  def property_submitted_for_review(property)
    ...
    mail(to: recipients, subject: 'Property Submitted for Review on AlveoHive') do |format|
      format.text
    end
  end
end
